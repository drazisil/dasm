import { ExtendedBuffer, shiftBuffer} from '../src/bufferTools'
...
describe('ExtendedBuffer class', () => {
    ...
    it('returns a value', () => {
        expect(extendedTestBuffer.shiftBuffer(7).toString()).toEqual('I_am_a_')
    })
    ...
    it('return a value and remainder that combine to match inputBuffer', () => {
        const value = extendedTestBuffer.shiftBuffer(7)
        expect(Buffer.concat([value.rawBuffer, extendedTestBuffer.rawBuffer]).toString()).toEqual(testBuffer.toString())
    })
